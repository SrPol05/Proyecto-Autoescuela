...
public class TeacherCalendarInterface
{
    ...
    private void seleccionarProfesorYMostrarInterfaz()
    {
        ...
        {
            int idp = Integer.parseInt(profesor[0]);
            ...
            {
                dialogoProfesor.dispose();
                mostrarInterfazIngresoFechas(idp);
            });
            row++;
        }
    ...
    private List<String[]> obtenerProfesores()
    {
        ...
        {
            String sql = "SELECT IDP, NOMBRE, APELLIDOS FROM Profesores";
    ...
    private void mostrarInterfazIngresoFechas(int professorIdp)
    {
        ...
        botonGuardar.addActionListener(e ->
        {
            ...
            if (guardarFecha(professorIdp, fecha))
            {
    ...
    private boolean guardarFecha(int professorIdp, String fecha)
    {
        System.out.println("Intentando guardar: IDP=" + professorIdp + ", Fecha=" + fecha);
        if (professorIdp <= 0)
        {
            System.out.println("IDP inválido: " + professorIdp);
            JOptionPane.showMessageDialog(ventana, "El ID del profesor es inválido.", "Error", JOptionPane.ERROR_MESSAGE);
            return false;
        }
        try (Connection conn = DriverManager.getConnection(URL_BASE_DATOS, USUARIO, CONTRASEÑA))
        {
            String checkSql = "SELECT COUNT(*) FROM Calendario WHERE IDP = ? AND FECHAS_DISPONIBLES = ?";
            PreparedStatement checkStmt = conn.prepareStatement(checkSql);
            checkStmt.setInt(1, professorIdp);
            checkStmt.setString(2, fecha);
            ResultSet rs = checkStmt.executeQuery();
            if (rs.next() && rs.getInt(1) > 0)
            {
                System.out.println("Entrada duplicada encontrada para IDP=" + professorIdp + ", Fecha=" + fecha);
                JOptionPane.showMessageDialog(ventana, "La fecha ya está registrada para este profesor.", "Error", JOptionPane.ERROR_MESSAGE);
                rs.close();
                checkStmt.close();
                return false;
            }
        ...
        {
            System.out.println("SQLException: " + e.getMessage() + ", ErrorCode: " + e.getErrorCode());
