...
public class TestInterface {
        ...
        try (Connection conn = DriverManager.getConnection(URL_BASE_DATOS, USUARIO, CONTRASEÑA)) {
            ...
            while (rsQuestions.next()) {
                ...
                if (respuestas.size() == 3) {
                    preguntas.add(new Question(idPregunta, preguntaTexto, respuestas, correccionesCompletas));
                }
            }
    ...
    private void mostrarPregunta() {
        ...
        Question pregunta = preguntas.get(preguntaActual);
    ...
    private void guardarResultados() {
        try (Connection conn = DriverManager.getConnection(URL_BASE_DATOS, USUARIO, CONTRASEÑA)) {
            ...
            for (int i = 0; i < preguntas.size(); i++) {
                Question pregunta = preguntas.get(i);
                String correccion = respuestasSeleccionadas.get(i);
                pstmt.setInt(1, nextCorrectId + i); // Unique CorrectID
    ...
    private static class Question {
        ...
        Question(int idPregunta, String textoPregunta, List<String> respuestas, List<String> correccionesCompletas) {
